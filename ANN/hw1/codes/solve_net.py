...
def train_net(model, loss, config, inputs, labels, batch_size, disp_freq, epoch, name):
    ...
    for input, label in data_iterator(inputs, labels, batch_size):
        ...
        if iter_counter % disp_freq == 0:
            wandb.log({f"{prefix}_{config.hiddenlayers}layers_train_loss": np.mean(loss_list), f"{prefix}_{config.hiddenlayers}layers_train_acc": np.mean(acc_list)}, step=epoch)
...
def test_net(model, loss, config, inputs, labels, batch_size, epoch, name):
    prefix = name.split("_")[0]
    ...
    wandb.log({f"{prefix}_{config.hiddenlayers}layers_test_loss": np.mean(loss_list), f"{prefix}_{config.hiddenlayers}layers_test_acc": np.mean(acc_list)}, step=epoch)
