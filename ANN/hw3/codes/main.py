...
def main():
    ...
    wandb.init(project='ANN-hw3', config=args, name=args.name)
    set_seed(1229)
    ...
    if args.test is None:
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            wandb.log({"train_loss": train_loss, "val_loss": val_loss, "val_ppl": val_ppl}, step=epoch)
        ...
        with open(f"output_{args.decode_strategy}.txt", "w") as fout:
            fout.write(f"test_loss: {test_loss}, test_ppl: {test_ppl}\n")
            fout.write("forward BLEU-4 {:.3f}, backward BLEU-4 {:.3f}, harmonic BLEU-4 {:.3f}\n".format(eval_result["fw-bleu-4"], eval_result["bw-bleu-4"], eval_result["fw-bw-bleu-4"]))
