...
def attn_selector(attn_type, config, W_q=None, W_k=None, W_v=None):
    ...
    elif attn_type.startswith("flash"):
        from models.attention_flashattention import FlashAttention
...
class Attention(nn.Module):
    ...
    def forward(self, X, mask):
        ...
        if self.attn_type.startswith("longformer") or self.attn_type.startswith("reformer"):
            ...
            Q = self.split_heads(self.W_q(X)).to(torch.bfloat16)
            K = self.split_heads(self.W_k(X)).to(torch.bfloat16)
            V = self.split_heads(self.W_v(X)).to(torch.bfloat16)
            with torch.cuda.amp.autocast(enabled = False):
